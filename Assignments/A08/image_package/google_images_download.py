...
class googleimagesdownload:
    ...
    def create_directories(self,main_directory, dir_name,thumbnail):
        ...
        try:
            if not os.path.exists(main_directory):
                ...
                sub_directory = sub_directory.replace(' ','_')
                if not os.path.exists(sub_directory):
                    os.makedirs(sub_directory)
                
                sub_directory_thumbnail = os.path.join(main_directory, dir_name_thumbnail).replace(' ','_')
    ...
    def download_image_thumbnail(self,image_url,main_directory,dir_name,return_image_name,print_urls,socket_timeout,print_size,no_download):
        ...
        try:
            ...
            try:
                ...
                path = main_directory + "/" + dir_name + "/" + return_image_name + ".jpg"
                path = path.replace(' ','_')
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download):
        ...
        if no_download:
            pass 
        # edited by Terry to NOT download the files, we only want
        # the thumbnail
        return "success","Printed url without downloading",count,None
        try:
            req = Request(image_url, headers={
                "User-Agent": "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"})
            try:
        ...
        while count < limit+1:
            ...
            if object == "no_links":
                ...
                if download_status == "success":
                    ...
                    download_status, download_message_thumbnail = self.download_image_thumbnail(object['image_thumbnail_url'],main_directory,dir_name,str(count),arguments['print_urls'],arguments['socket_timeout'],arguments['print_size'],arguments['no_download'])
